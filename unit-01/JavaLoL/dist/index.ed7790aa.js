...
const getChampions = async ()=>{
    ...
    await fetch("https://ddragon.leagueoflegends.com/cdn/13.18.1/data/es_ES/champion.json").then(function(result) {
        ...
        const data = result;
        const champion = new (0, _championJsDefault.default)(data);
...
const showChampions = async ()=>{
    ...
    for(var i = 0; i < championsList.length; i++){
        ...
        while(aux != championsList[i].pkm_type.length){
            if (aux == 0) var tipo1 = championsList[i].pkm_type[aux].type.name;
            if (aux == 1) var tipo2 = championsList[i].pkm_type[aux].type.name;
            else tipo2 = "";
            aux++;
        }
